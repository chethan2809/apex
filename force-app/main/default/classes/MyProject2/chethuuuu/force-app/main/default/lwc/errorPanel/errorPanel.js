        ...
        return (
            errors
                ...
                .map(error => {
                    ...
                    // ErrException from Err.addError()
                    else if (error.detail && error.detail.output && Array.isArray(error.detail.output.errors)) {
                        return error.detail.output.errors.map(e => e.message);
                    }
                    // Array of unknown errors shape so try HTTP status text
                    else if (Array.isArray(error)){
                        return error.map(e => e.statusText);
                    }
